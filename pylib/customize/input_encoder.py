...
class NoInputEncoder(BaseInputEncoding):
    def apply(self, X, prefix:str=None):
        ...
        if self.model_input_specification.categorical_format == CategoricalFormat.Integers:
            ...
            categorical_feature_inputs = [Lambda(lambda x: tf.cast(x, tf.float32))(c)
                                          for c in categorical_feature_inputs]
...
class RecordLevelEmbed(BaseInputEncoding):
    ...
    def apply(self, X:List[keras.Input], prefix: str = None):
        ...
        x = Dense(self.embed_dimension, activation="linear",
                  use_bias=not self.project, name=f"{prefix}embed")(x)
...
class CategoricalFeatureEmbed(BaseInputEncoding):
    ...
    def apply(self, X:List[keras.Input], prefix:str=None):
        ...
        for categorical_field_i, categorical_field_name in enumerate(self.model_input_specification.categorical_feature_names):
            ...
            if self.embed_layer_type != EmbedLayerType.Lookup:
                ...
                x = cat_field_x
                x = Embedding(input_dim=self.model_input_specification.\
                              levels_per_categorical_feature[categorical_field_i] + 1,
                              output_dim=self.dimensions_per_feature, input_length=self.sequence_length)(x)
                x = Reshape((self.sequence_length, self.dimensions_per_feature),
                            name=f"{prefix}expand_{categorical_field_name}")(x)
    ...
    def required_input_format(self) -> CategoricalFormat:
        return CategoricalFormat.Integers \
            if self.embed_layer_type == EmbedLayerType.Lookup else CategoricalFormat.OneHot
